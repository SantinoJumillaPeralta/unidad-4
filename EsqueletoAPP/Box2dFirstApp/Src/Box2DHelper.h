...
public:
	...
	Cannon::Cannon(b2World* world, RenderWindow* wnd) : window(wnd), power(0.0f) {
		// Crear el cuerpo del cañón
		b2BodyDef bodyDef;
		bodyDef.type = b2_staticBody; // El cañón no se moverá
		bodyDef.position.Set(100.0f, 90.0f); // Posición inicial
		cannonBody = world->CreateBody(&bodyDef);

		// Crear la forma del cañón (puedes usar un polígono o una cadena de círculos)
		b2CircleShape cannonShape;
		cannonShape.m_radius = 5.0f; // Tamaño del cañón
		b2FixtureDef fixtureDef;
	...
	void Cannon::update(sf::Vector2f mousePosition) {
		...
		float angle = atan2(dy, dx); // Calcular ángulo en radianes
